...
# Managed by pbs - updates via `pbs sync`
#
...
setup_vlmail() {
    ...
    local VLMAIL_REPO="/srv/fast/code/vl-mail"

    # Check if vl-mail repo is available
    if [[ ! -d "$VLMAIL_REPO" ]]; then
        log_feature "vl-mail" "vl-mail repo not found at $VLMAIL_REPO, skipping"
        return 0
    fi
    ...
    if [[ ! -f "$VLMAIL_BIN" ]] || [[ "$VLMAIL_REPO/main.go" -nt "$VLMAIL_BIN" ]] || [[ "$VLMAIL_REPO/go.mod" -nt "$VLMAIL_BIN" ]]; then
        log_feature "vl-mail" "Building vl-mail..."
        if (cd "$VLMAIL_REPO" && $GO_BIN build -o "$VLMAIL_BIN" .); then
